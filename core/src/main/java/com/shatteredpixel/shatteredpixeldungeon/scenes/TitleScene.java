...
//		add(btnSupport);
		...
		if (landscape()) {
			...
			//btnSupport.setRect(btnNews.right()+2, btnNews.top(), btnRankings.width(), BTN_HEIGHT);
			btnAbout.setRect(btnNews.right()+2, btnRankings.bottom() + GAP, btnSettings.width(), BTN_HEIGHT);
		} else {
...
//		if (landscape()) {
//			btnPlay.setRect(((title.x+(title.width()/2))-((title.width()+150)/4)), topRegion+GAP, ((title.width()+150)/2), BTN_HEIGHT);
//			align(btnPlay);
//			//btnSupport.setRect(btnPlay.right()+2, btnPlay.top(), btnPlay.width(), BTN_HEIGHT);
//			btnRankings.setRect(title.x-50, btnPlay.bottom()+ GAP, ((((title.width()+100)/2)-1)*.67f)-1, BTN_HEIGHT);
